...
// Reusable subscriber + publisher template.
// Future user should replace:
// - input/output message types if Float32 is not suitable.
// - handle_input() with real processing logic.
// - publish_output() with real result publishing logic.
...
class RegistrableSubscriberTemplateNode : public rclcpp::Node
{
public:
  RegistrableSubscriberTemplateNode()
  : Node("registrable_subscriber_template_node")
  {
    declare_parameters();
    read_parameters();
    setup_qos();
    ...
    RCLCPP_INFO(get_logger(), "Registrable subscriber template node started");
  }
...
private:
  ...
  {
    declare_parameter<std::string>("module_name", "subscriber_template_node");
    declare_parameter<bool>("critical", false);
    declare_parameter<std::string>("input_topic", "/template/input");
    declare_parameter<std::string>("heartbeat_topic", "/template_subscriber/heartbeat");
    ...
    declare_parameter<std::string>("output_topic", "/template_subscriber/output");
    declare_parameter<int>("output_deadline_ms", 500);
    declare_parameter<int>("auto_deregister_after_cycles", 0);
    declare_parameter<std::string>(
        "request_deregister_service",
        "/template_subscriber/request_deregister");
  ...
  void read_parameters()
  {
    ...
    output_deadline_ms_ = get_parameter("output_deadline_ms").as_int();
    ...
    if (module_name_.empty() || input_topic_.empty() || heartbeat_topic_.empty()) {
      throw std::runtime_error("module_name, input_topic, and heartbeat_topic must not be empty");
    }

    if (input_topic_.front() != '/' || heartbeat_topic_.front() != '/') {
      throw std::runtime_error("topics must start with /");
    }
    ...
    if (output_topic_.empty() || output_topic_.front() != '/') {
      throw std::runtime_error("output_topic must start with /");
    }

    if (output_deadline_ms_ <= 0) {
      throw std::runtime_error("output_deadline_ms must be greater than 0");
    }
  ...
  void setup_qos()
  {
    ...
    output_qos_ = rclcpp::QoS(rclcpp::KeepLast(10)).reliable()
      .deadline(std::chrono::milliseconds(output_deadline_ms_));
  ...
  {
    input_subscription_ = create_subscription<std_msgs::msg::Float32>(
        input_topic_,
        rclcpp::QoS(rclcpp::KeepLast(10)).reliable(),
        std::bind(&RegistrableSubscriberTemplateNode::handle_input, this, std::placeholders::_1));
    ...
    output_publisher_ = create_publisher<std_msgs::msg::Float32>(
        output_topic_,
        output_qos_);
    ...
    request_deregister_service_ = create_service<Trigger>(
        request_deregister_service_name_,
        std::bind(
          &RegistrableSubscriberTemplateNode::handle_request_deregister,
          this,
          std::placeholders::_1,
          std::placeholders::_2));
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&RegistrableSubscriberTemplateNode::timer_callback, this));
  }
  ...
  // Replace this function with real processed-output publishing logic.
  ...
  void publish_output()
  {
    ...
    output.data = latest_input_ * 2.0F;
...
// Replace this callback with real input processing logic.

  void handle_input(const std_msgs::msg::Float32::SharedPtr msg)
  {
  ...
  void publish_heartbeat()
  {
    ...
    if (has_input_) {
      heartbeat.data += " input=" + std::to_string(latest_input_);
    }
  ...
  int output_deadline_ms_;
  float latest_input_{0.0F};
  ...
  rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr input_subscription_;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<RegistrableSubscriberTemplateNode>());
  rclcpp::shutdown();
  return 0;
}
