...
private:
  ...
  void read_parameters()
  {
    ...
    if (publish_data_topic_ && data_topic_.front() != '/') {
      throw std::runtime_error("data_topic must start with /");
    }

    if (publish_data_topic_ && data_deadline_ms_ <= 0) {
      throw std::runtime_error("data_deadline_ms must be greater than 0");
    }
  ...
  void setup_qos()
  {
    ...
    data_qos_ = rclcpp::QoS(rclcpp::KeepLast(10)).reliable();

    if (publish_data_topic_) {
      data_qos_.deadline(std::chrono::milliseconds(data_deadline_ms_));
    }
  ...
  void setup_publishers()
  {
    ...
    if (publish_data_topic_) {
      data_publisher_ = create_publisher<std_msgs::msg::Float32>(
        data_topic_,
        data_qos_);
    }
  }
  ...
  void request_register()
  {
    ...
    if (publish_data_topic_) {
      MonitorSpec data_monitor;
      data_monitor.topic_name = data_topic_;
      data_monitor.kind = "data";
      data_monitor.message_type = "std_msgs/msg/Float32";
      data_monitor.reliability = "reliable";
      data_monitor.deadline_ms = data_deadline_ms_;
      data_monitor.liveliness_ms = 0;

      request->monitors.push_back(data_monitor);
    }
  ...
  void publish_data()
  {
    ...
    value.data = static_cast<float>(data_counter_++);
    data_publisher_->publish(value);
  }
  ...
  rclcpp::QoS data_qos_{rclcpp::KeepLast(10)};
